...
fn main() {
    let argsL Vec<_> = env::args().collect();
    if args.len() != 2 {
    ...
    if args[1] == "--server" {
        ...
        let mut stream = TcpStream::connect("127.0.0.1:8888").expect("Could not connect to sever");
        ...
        loop {
            ...
            let input = str::from_utf8(&bufer)
                .expect("Could not write buffer as string");
            if input == "" {
                eprintln!("Emptry response from server");
            }
